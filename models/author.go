...
type authorR struct {
	ArtistProjects ProjectSlice `boil:"ArtistProjects" json:"ArtistProjects" toml:"ArtistProjects" yaml:"ArtistProjects"`
	AuthorProjects ProjectSlice `boil:"AuthorProjects" json:"AuthorProjects" toml:"AuthorProjects" yaml:"AuthorProjects"`
}
...
var (
	authorAllColumns            = []string{"id", "slug", "name"}
	authorColumnsWithoutDefault = []string{}
	authorColumnsWithDefault    = []string{"id", "slug", "name"}
...
// ArtistProjects retrieves all the project's Projects with an executor via id column.
func (o *Author) ArtistProjects(mods ...qm.QueryMod) projectQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"project_artists\" on \"project\".\"id\" = \"project_artists\".\"project_id\""),
		qm.Where("\"project_artists\".\"artist_id\"=?", o.ID),
	)
...
// AuthorProjects retrieves all the project's Projects with an executor via id column.
func (o *Author) AuthorProjects(mods ...qm.QueryMod) projectQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"project_authors\" on \"project\".\"id\" = \"project_authors\".\"project_id\""),
		qm.Where("\"project_authors\".\"author_id\"=?", o.ID),
	)
...
// LoadArtistProjects allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (authorL) LoadArtistProjects(e boil.Executor, singular bool, maybeAuthor interface{}, mods queries.Applicator) error {
	var slice []*Author
	var object *Author
	...
	query := NewQuery(
		qm.Select("\"project\".id, \"project\".slug, \"project\".locked, \"project\".created_at, \"project\".updated_at, \"project\".published_at, \"project\".title, \"project\".description, \"project\".cover_id, \"project\".project_status, \"project\".series_status, \"project\".demographic, \"project\".rating, \"a\".\"artist_id\""),
		qm.From("\"project\""),
		qm.InnerJoin("\"project_artists\" as \"a\" on \"project\".\"id\" = \"a\".\"project_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Slug, &one.Locked, &one.CreatedAt, &one.UpdatedAt, &one.PublishedAt, &one.Title, &one.Description, &one.CoverID, &one.ProjectStatus, &one.SeriesStatus, &one.Demographic, &one.Rating, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for project")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice project")
		}
	...
	if singular {
		object.R.ArtistProjects = resultSlice
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.ArtistProjects = append(local.R.ArtistProjects, foreign)
				if foreign.R == nil {
					foreign.R = &projectR{}
				}
...
// LoadAuthorProjects allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (authorL) LoadAuthorProjects(e boil.Executor, singular bool, maybeAuthor interface{}, mods queries.Applicator) error {
	var slice []*Author
	var object *Author
	...
	query := NewQuery(
		qm.Select("\"project\".id, \"project\".slug, \"project\".locked, \"project\".created_at, \"project\".updated_at, \"project\".published_at, \"project\".title, \"project\".description, \"project\".cover_id, \"project\".project_status, \"project\".series_status, \"project\".demographic, \"project\".rating, \"a\".\"author_id\""),
		qm.From("\"project\""),
		qm.InnerJoin("\"project_authors\" as \"a\" on \"project\".\"id\" = \"a\".\"project_id\""),
	...
	if singular {
		object.R.AuthorProjects = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &projectR{}
			}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.AuthorProjects = append(local.R.AuthorProjects, foreign)
				if foreign.R == nil {
					foreign.R = &projectR{}
				}
...
// AddArtistProjects adds the given related objects to the existing relationships
// of the author, optionally inserting them as new records.
// Appends related to o.R.ArtistProjects.
// Sets related.R.Artists appropriately.
func (o *Author) AddArtistProjects(exec boil.Executor, insert bool, related ...*Project) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"project_artists\" (\"artist_id\", \"project_id\") values ($1, $2)"
	...
	if o.R == nil {
		o.R = &authorR{
			ArtistProjects: related,
		}
	} else {
		o.R.ArtistProjects = append(o.R.ArtistProjects, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &projectR{
				Artists: AuthorSlice{o},
			}
		} else {
...
// SetArtistProjects removes all previously related items of the
// author replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Artists's ArtistProjects accordingly.
// Replaces o.R.ArtistProjects with related.
// Sets related.R.Artists's ArtistProjects accordingly.
func (o *Author) SetArtistProjects(exec boil.Executor, insert bool, related ...*Project) error {
	query := "delete from \"project_artists\" where \"artist_id\" = $1"
	...
	removeArtistProjectsFromArtistsSlice(o, related)
	if o.R != nil {
		o.R.ArtistProjects = nil
	}
	return o.AddArtistProjects(exec, insert, related...)
}

// RemoveArtistProjects relationships from objects passed in.
// Removes related items from R.ArtistProjects (uses pointer comparison, removal does not keep order)
// Sets related.R.Artists.
func (o *Author) RemoveArtistProjects(exec boil.Executor, related ...*Project) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"project_artists\" where \"artist_id\" = $1 and \"project_id\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.ArtistProjects {
			if rel != ri {
				continue
			}

			ln := len(o.R.ArtistProjects)
			if ln > 1 && i < ln-1 {
				o.R.ArtistProjects[i] = o.R.ArtistProjects[ln-1]
			}
			o.R.ArtistProjects = o.R.ArtistProjects[:ln-1]
			break
		}
	}
...
func removeArtistProjectsFromArtistsSlice(o *Author, related []*Project) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// AddAuthorProjects adds the given related objects to the existing relationships
// of the author, optionally inserting them as new records.
// Appends related to o.R.AuthorProjects.
// Sets related.R.Authors appropriately.
func (o *Author) AddAuthorProjects(exec boil.Executor, insert bool, related ...*Project) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"project_authors\" (\"author_id\", \"project_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
	...
	if o.R == nil {
		o.R = &authorR{
			AuthorProjects: related,
		}
	} else {
		o.R.AuthorProjects = append(o.R.AuthorProjects, related...)
	}
...
// SetAuthorProjects removes all previously related items of the
// author replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Authors's AuthorProjects accordingly.
// Replaces o.R.AuthorProjects with related.
// Sets related.R.Authors's AuthorProjects accordingly.
func (o *Author) SetAuthorProjects(exec boil.Executor, insert bool, related ...*Project) error {
	query := "delete from \"project_authors\" where \"author_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	removeAuthorProjectsFromAuthorsSlice(o, related)
	if o.R != nil {
		o.R.AuthorProjects = nil
	}
	return o.AddAuthorProjects(exec, insert, related...)
}

// RemoveAuthorProjects relationships from objects passed in.
// Removes related items from R.AuthorProjects (uses pointer comparison, removal does not keep order)
// Sets related.R.Authors.
func (o *Author) RemoveAuthorProjects(exec boil.Executor, related ...*Project) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"project_authors\" where \"author_id\" = $1 and \"project_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
	...
	for _, rel := range related {
		for i, ri := range o.R.AuthorProjects {
			if rel != ri {
				continue
			}

			ln := len(o.R.AuthorProjects)
			if ln > 1 && i < ln-1 {
				o.R.AuthorProjects[i] = o.R.AuthorProjects[ln-1]
			}
			o.R.AuthorProjects = o.R.AuthorProjects[:ln-1]
			break
		}
	}
...
func removeAuthorProjectsFromAuthorsSlice(o *Author, related []*Project) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func FindAuthor(exec boil.Executor, iD int64, selectCols ...string) (*Author, error) {
	...
	if err = authorObj.doAfterSelectHooks(exec); err != nil {
...
func (o *Author) Update(exec boil.Executor, columns boil.Columns) error {
	var err error
	...
	if !cached {
		...
		if len(wl) == 0 {
			return errors.New("models: unable to update author, could not build whitelist")
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to update author row")
	}
...
func (q authorQuery) UpdateAll(exec boil.Executor, cols M) error {
	queries.SetUpdate(q.Query, cols)

	_, err := q.Query.Exec(exec)
	if err != nil {
		return errors.Wrap(err, "models: unable to update all for author")
	}
...
func (o *Author) Delete(exec boil.Executor) error {
	...
	_, err := exec.Exec(sql, args...)
	if err != nil {
		return errors.Wrap(err, "models: unable to delete from author")
	}
...
func (q authorQuery) DeleteAll(exec boil.Executor) error {
	if q.Query == nil {
	...
	_, err := q.Query.Exec(exec)
	if err != nil {
		return errors.Wrap(err, "models: unable to delete all from author")
	}
